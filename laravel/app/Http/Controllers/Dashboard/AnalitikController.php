...
class AnalitikController extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        if ($post && $ajax) {
            ...
            $id = $post['id'];
			$getVideo = $this->model_video->where('id', $id)->first();
            $this->parse['total_view']  = $this->model_shopeback->where('status',1)->where('video_id',$id)->whereBetween('created_at', [ $startDate->toDateString().' 00:00:00', $nowDate->toDateString().' 23:59:59'])->count();
            ...
            $this->parse['avg']  = (int) $this->model_shopeback ->selectRaw('AVG(TIME_TO_SEC(duration)) as avg')->where('status',1)->where('video_id',$id)->whereBetween('created_at', [ $startDate->toDateString().' 00:00:00', $nowDate->toDateString().' 23:59:59'])->get()[0]['avg'];
